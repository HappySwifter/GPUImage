    ...
    if (_hasAudioTrack)
    {
        ...
        void(^write)(void) = ^() {
            ...
            if (!self->assetWriterAudioInput.readyForMoreMediaData)
            {
                ...
                if(self->assetWriter.status == AVAssetWriterStatusWriting && CMSampleBufferGetNumSamples(audioBuffer))
                {
                    if (![self->assetWriterAudioInput appendSampleBuffer:audioBuffer])
                        NSLog(@"Problem appending audio buffer at time: %@", CFBridgingRelease(CMTimeCopyDescription(kCFAllocatorDefault, currentSampleTime)));
                } else
                {
                    NSLog(@"assetWriter.status %ld", (long)self->assetWriter.status);
...
- (void)enableSynchronizationCallbacks;
{
    if (videoInputReadyCallback != NULL)
    {
        ...
        [self->assetWriterVideoInput requestMediaDataWhenReadyOnQueue:videoQueue usingBlock:^{
            if( self->_paused )
            {
...
- (void)createDataFBO;
{
    ...
    if ([GPUImageContext supportsFastTextureUpload]) {
        ...
        CVReturn status = CVPixelBufferPoolCreatePixelBuffer (NULL, [assetWriterPixelBufferInput pixelBufferPool], &renderTarget);
